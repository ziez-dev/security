...
fn middlewareDeinit(ptr: ?*anyopaque, _: std.mem.Allocator) void {
    if (ptr) |p| std.heap.page_allocator.destroy(@as(*SecurityConfig, @ptrCast(@alignCast(p))));
}

pub fn asMiddleware(config: SecurityConfig) ziez.Middleware {
    const cfg = std.heap.page_allocator.create(SecurityConfig) catch @panic("ziez-security: OOM");
    cfg.* = config;
    return .{
        .ptr = cfg,
